...
export default function CustomSelectorManager({
  ...
  const createSelector = (event: any) => {
    if (event.key === 'Enter') {
      // const next = selectors.length + 1;

      const selectors = String(event.target.value).split(" ")

      for (let i = 0; i < selectors.length; i++) {
        addSelector({ name: `${selectors[i]}`, label: `${selectors[i]}` });
      }
    ...
    <div className="gjs-custom-selector-manager p-2 flex flex-col gap-2 text-left">
      ...
      >
        ...
        {targetStr ? (
          <>
            {showInput ?
              <input autoFocus type="text" className='bg-zinc-700 border-none w-full' onKeyDown={createSelector} />
              :
              <button
                type="button"
